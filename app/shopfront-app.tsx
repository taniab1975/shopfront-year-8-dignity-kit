...
export default function ShopfrontApp() {
  ...
  const writingMinutes = authenticMetrics.startedAt && clockNow
    ? Math.max(1, Math.round((clockNow - authenticMetrics.startedAt) / 60000))
    : 0;
  ...
  const subjectLensIdBySubject = useMemo(
    () =>
      lenses.reduce<Record<string, string>>((acc, lens) => {
        acc[lens.subject] = lens.id;
        return acc;
      }, {}),
    [],
  );

  function changeSubjectFilter(nextSubject: string) {
    setSubjectFilter(nextSubject);
    const matchingLensId = subjectLensIdBySubject[nextSubject];
    if (matchingLensId) {
      setActiveLensId(matchingLensId);
    }
  }
      ...
      <div className="workspace">
        ...
        <section className="process-section" id="process">
          ...
          <article className="tool-panel" aria-label="Critical thinking record">
            ...
            <div className="decision-table" role="region" aria-label="Qualitative decision matrix">
              <table>
                <thead>
                  <tr>
                    <th>Criterion</th>
        ...
        <section className="curriculum-section" id="curriculum">
          ...
          <div className="filters">
            <label>
              Subject
              <select value={subjectFilter} onChange={(event) => changeSubjectFilter(event.target.value)}>
            ...
            <span className="filter-result-chip" aria-live="polite">
              Showing {filteredCurriculumRows.length} {filteredCurriculumRows.length === 1 ? "row" : "rows"}
            </span>
          </div>

          <div className="matrix-table" role="region" aria-label="Curriculum mapping table">
            <table>
              <thead>
                <tr>
                  <th>Subject</th>
        ...
        <section className="assessment-section" id="assessment">
          ...
          <div className="assessment-table" role="region" aria-label="Assessment rubric">
            <table>
              <thead>
                <tr>
                  <th>Lens</th>
